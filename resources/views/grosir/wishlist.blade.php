    ...
    <div class="relative isolate pt-14">
      <div class="bg-white">
        <div class="max-w-2xl px-4 py-16 mx-auto sm:px-6 sm:py-24 lg:max-w-7xl lg:px-8">
          <h2 class="text-2xl font-semibold text-gray-900">Produk Favorit Anda</h2>
            ...
            @forelse($wishlistsWithProducts as $item)
              <div class="group">
                <div class="relative w-full overflow-hidden bg-gray-200 rounded-lg aspect-h-1 aspect-w-1 xl:aspect-h-8 xl:aspect-w-7">
                  <img src="{{ asset($item['product']->foto_produk) }}" alt="{{ $item['product']->nama_produk }}" class="object-cover object-center w-full h-full group-hover:opacity-75">
                ...
                <h3 class="mt-4 text-sm text-gray-700">{{ $item['product']->nama_produk }}</h3>
                <p class="mt-1 text-lg font-medium text-gray-900">Rp{{ number_format($item['product']->harga_produk, 0, ',', '.') }}</p>
              </div>
            @empty
